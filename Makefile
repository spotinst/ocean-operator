...
# Image spec for testing.
IMAGE_REPOSITORY ?= spotinst
IMAGE_NAME       ?= ocean-operator
IMAGE_TAG        ?= $(VERSION)
IMAGE_SPEC       := $(IMAGE_REPOSITORY)/$(IMAGE_NAME):$(IMAGE_TAG)
...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	...
	$(Q) cp config/crd/bases/* pkg/tide/crds/
...
build-image: ## Build Docker image for all platforms.
...
.PHONY: release
release: manifests generate fmt ## Release a new version.
ifneq ($(strip $(GIT_DIRTY)),)
	...
	$(Q) echo $(shell bash -c 'read -p "Release version: $(VERSION) → " version; echo $$version') > internal/version/VERSION
