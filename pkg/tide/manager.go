...
func (m *manager) ApplyEnvironment(ctx context.Context, options ...ApplyOption) error {
	oceanSA, oceanBinding, err := m.loadRBAC(oceanv1alpha1.NamespaceSystem)
	if err != nil {
		return err
	}
	...
	oceanCRDs, err := m.loadCRDs(options...)
	if err != nil {
		return err
	}
	...
	oceanComponents, err := m.loadComponents(options...)
	if err != nil {
		return err
	}
...
func (m *manager) ApplyRBAC(ctx context.Context, sa *corev1.ServiceAccount,
	...
	if crb != nil {
		if err := m.applyRBACRoleBinding(ctx, crb, opts); err != nil {
			return err
		}
	}
	return nil
...
func (m *manager) applyRBACServiceAccount(ctx context.Context,
	...
	if sa.Namespace == "" {
		sa.Namespace = oceanv1alpha1.NamespaceSystem
	}

	existing := sa.DeepCopy()
	objKey := ctrlrt.ObjectKeyFromObject(sa)
	err := m.clientRuntime.Get(ctx, objKey, existing)
	switch {
	case apierrors.IsNotFound(err):
		m.log.Info("creating tide service account", "name", objKey.Name)
	...
	default:
		m.log.Info("patching tide service account", "name", objKey.Name)
		if err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
			if err := m.clientRuntime.Get(ctx, objKey, existing); err != nil {
				return err
			}
			sa.SetResourceVersion(existing.GetResourceVersion())
			return m.clientRuntime.Patch(ctx, sa, ctrlrt.MergeFrom(existing))
		}); err != nil {
			return err
		}
	}
...
func (m *manager) applyRBACRoleBinding(ctx context.Context,
	crb *rbacv1.ClusterRoleBinding, options *ApplyOptions) error {
	existing := crb.DeepCopy()
	objKey := ctrlrt.ObjectKeyFromObject(crb)
	err := m.clientRuntime.Get(ctx, objKey, existing)
	switch {
	case apierrors.IsNotFound(err):
		m.log.Info("creating tide cluster role binding", "name", objKey.Name)
	...
	default:
		m.log.Info("patching tide cluster role binding", "name", objKey.Name)
		if err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
			if err := m.clientRuntime.Get(ctx, objKey, existing); err != nil {
				return err
			}
			crb.SetResourceVersion(existing.GetResourceVersion())
			return m.clientRuntime.Patch(ctx, crb, ctrlrt.MergeFrom(existing))
		}); err != nil {
			return err
		}
	}
...
func (m *manager) applyCRD(ctx context.Context,
	...
	if crd.Namespace == "" {
		crd.Namespace = oceanv1alpha1.NamespaceSystem
	}

	existing := crd.DeepCopy()
	objKey := ctrlrt.ObjectKeyFromObject(crd)
	err := m.clientRuntime.Get(ctx, objKey, existing)
	switch {
	case apierrors.IsNotFound(err):
		m.log.Info("creating ocean crd", "name", objKey.Name)
	...
	default:
		m.log.Info("patching ocean crd", "name", objKey.Name)
		if err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
			if err := m.clientRuntime.Get(ctx, objKey, existing); err != nil {
				return err
			}
			crd.SetResourceVersion(existing.GetResourceVersion())
			return m.clientRuntime.Patch(ctx, crd, ctrlrt.MergeFrom(existing))
		}); err != nil {
			return err
		}
	}
...
func (m *manager) applyComponent(ctx context.Context,
	...
	if component.Spec.State == oceanv1alpha1.OceanComponentStateAbsent {
		m.log.Info("skipping ocean component", "name", component.Name)
		return nil
	}
	if component.Namespace == "" {
		component.Namespace = oceanv1alpha1.NamespaceSystem
	}

	existing := component.DeepCopy()
	objKey := ctrlrt.ObjectKeyFromObject(component)
	err := m.clientRuntime.Get(ctx, objKey, existing)
	switch {
	case apierrors.IsNotFound(err):
		m.log.Info("creating ocean component", "name", objKey.Name)
	...
	default:
		m.log.Info("patching ocean component", "name", objKey.Name)
		if err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
			if err := m.clientRuntime.Get(ctx, objKey, existing); err != nil {
				return err
			}
			...
			return m.clientRuntime.Patch(ctx, component, ctrlrt.MergeFrom(existing))
		}); err != nil {
			return err
		}
	}
...
func (m *manager) DeleteEnvironment(ctx context.Context, options ...DeleteOption) error {
	...
	if err := m.clientRuntime.List(ctx, componentList); err != nil {
		...
		if ok {
			m.log.Info("ocean components are not present", "message", componentGone.Error())
		} else {
			return err
		}
	}
	...
	crdFieldSet := ctrlrt.MatchingFields{
...
func (m *manager) deleteRBACServiceAccount(ctx context.Context, name string, options *DeleteOptions) error {
	m.log.Info("deleting tide service account")
...
func (m *manager) deleteRBACRoleBinding(ctx context.Context, name string, options *DeleteOptions) error {
	m.log.Info("deleting tide cluster role binding")
...
func (m *manager) deleteCRD(ctx context.Context,
	...
	m.log.Info("deleting ocean crd", "name", crd.Name)
...
func (m *manager) deleteComponent(ctx context.Context,
	...
	m.log.Info("deleting ocean component", "name", component.Name)
...
func (m *manager) loadCRDs(options ...ApplyOption) ([]*apiextensionsv1.CustomResourceDefinition, error) {
	m.log.Info("loading ocean crds")
...
func (m *manager) loadCRD(name string) (*apiextensionsv1.CustomResourceDefinition, error) {
	...
	m.log.V(4).Info("loaded crd", "crd", crd)
	return crd, nil
}

func (m *manager) loadComponents(options ...ApplyOption) ([]*oceanv1alpha1.OceanComponent, error) {
	m.log.Info("loading ocean components")
	...
	for _, manifest := range manifests {
		...
		if _, present := opts.ComponentsFilter[comp.Spec.Name]; present { // component name
			comp.Spec.State = oceanv1alpha1.OceanComponentStatePresent
		} else if _, present = opts.ComponentsFilter[oceanv1alpha1.OceanComponentName(comp.Name)]; present { // resource name
			comp.Spec.State = oceanv1alpha1.OceanComponentStatePresent
		} else {
...
func (m *manager) loadRBAC(namespace string) (*corev1.ServiceAccount, *rbacv1.ClusterRoleBinding, error) {
	m.log.Info("loading ocean rbac resources")

	manifests, err := tiderbac.GetRBACManifests(namespace)
	if err != nil {
...
func (m *manager) waitForCRD(ctx context.Context, crd *apiextensionsv1.CustomResourceDefinition) error {
	return wait.PollImmediate(time.Second, time.Minute, func() (bool, error) {
		m.log.Info("waiting for ocean crd to be available", "name", crd.Name, "namespace", crd.Namespace)
		existing := crd.DeepCopy()
		objName := ctrlrt.ObjectKeyFromObject(crd)
